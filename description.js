...
const seat_div = document.querySelector('#seat')
seat_div.classList.add('seat_container')
seatFunction(seats)
function seatFunction(seats){
  seats.forEach((seat)=>{
    console.log(seat.id)

    const seatEl = document.createElement('button')
    seatEl.classList.add('seat_items')
    seatEl.innerHTML = `
    <div>
    <div>${seat.id}</div>
    </div>
    `
    seat_div.appendChild(seatEl)
